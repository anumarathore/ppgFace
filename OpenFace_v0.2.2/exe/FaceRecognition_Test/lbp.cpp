...
template <typename _Tp>
void lbp::FPLBP_(const Mat &img, Mat &features,int radius)
{
	Mat_<uchar> I(img);
...
void lbp::FPLBP(const cv::Mat &img, Mat &features, int radius){
	switch (img.type()) {
	case CV_8SC1: FPLBP_<char>(img, features, radius); break;
	case CV_8UC1: FPLBP_<unsigned char>(img, features, radius); break;
	case CV_16SC1: FPLBP_<short>(img, features, radius); break;
	case CV_16UC1: FPLBP_<unsigned short>(img, features, radius); break;
	case CV_32SC1: FPLBP_<int>(img, features, radius); break;
	case CV_32FC1: FPLBP_<float>(img, features, radius); break;
	case CV_64FC1: FPLBP_<double>(img, features, radius); break;
	}
}
...
Mat lbp::FPLBP(const cv::Mat &img, int radius ){ Mat dst; FPLBP(img, dst, radius);  return dst; }
